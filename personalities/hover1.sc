...
//------------------------------------------------------------
SynthDef(\hoverLoop, { |out = 0, bufnum = 0, amp = 0, rate = 1, t_trig = 0, gate = 1|
...
~init = ~init <> { |d|
	...
	var folder = PathName("~/Downloads/yourDNASamples/hover");
	...
	fork {
		s.sync;
		synth = Synth(\hoverLoop, [\bufnum, buffers[bi], \amp, 0, \gate, 1]);
...
~next = { |d|
	...
	var drive = m.accelMassFiltered;
	var amp = drive.lincurve(0, 0.6, 0, 1, 2);
	var rate = drive.linlin(0, 0.6, 1, 1.22);

	synth !? { synth.set(\amp, amp, \rate, rate) };

	if(armed and: { m.accelMass > 0.55 }, {
		armed = false;
		bi = (bi + 1) % buffers.size;
		synth !? { synth.set(\bufnum, buffers[bi], \t_trig, 1) };
	...
	if(m.accelMass < 0.25, { armed = true });
};
