...
SynthDef(\glideSINE, { |out = 0, freq = 220, amp = 0.3, vel = 1.0,
	decay = 1.5, coef = 0.3, body = 0.6, tone = 4000,
	gate = 1, rel = 1.5, pan = 0, glide = 0.12, t_trig = 0|

	var sig = SinOsc.ar(freq.lagud(0.8,0.8), 0, 0.4);
	var strike = Decay2.kr(t_trig * vel, 0.001, 2.8);
	var env = EnvGen.kr(Env.cutoff(rel, 1, \sin), gate, doneAction: 2);

	Out.ar(out, Pan2.ar(sig * strike * env * amp, Latch.kr(pan, t_trig)).tanh);
...
SynthDef(\hollowWood, { |out = 0, freq = 220, amp = 0.3, vel = 1.0,
	...
	var plk = Pluck.ar(exc, fire, 0.05, freq.reciprocal.clip(0.0001, 0.05) * 0.25,
		decay, coef.clip(0.0, 0.95));

	var t = SinOsc.ar(freq.lagud(0.8, 0.2)	 * 2,0,0.4) * EnvGen.kr(Env.perc(0.001, 2.8), fire);
	var knock = Decay2.ar(fire, 0.0005, 0.008) * PinkNoise.ar(1);
	var bod = DynKlank.ar(`[
		[freq * 0.5, freq * 1.34, freq * 2.71],
		[0.5, 0.25, 0.12],
		[0.30, 0.18, 0.10] * body
	], knock);

	var sig = (plk + bod + t) * vel;
	var env = EnvGen.kr(Env.cutoff(rel, 1, \sin), gate, doneAction: 2);
...
~init = ~init <> {
	...
	voice = Synth(\glideSINE, [\amp, 0.35, \rel, relTime, \glide, glide], group);
...
*/
			...
			bod  = rate.linlin(0.5, 12.0, 2.0, 0.3);
