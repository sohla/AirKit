...
~next = {|d|
	...
	var oct = m.gyroYFiltered.linlin(-1,1,8,4).floor;
  	var envRel = m.accelMassFiltered.lincurve(0, 5 * sens,0.4,1.1,1);
	var amp = m.accelMassFiltered.lincurve(0, 4.2 * sens,0.001,1.0,-1);
	var ff = (d.sensors.gyroEvent.y / pi.half).linexp(-1,1,80,14000,-2);
	var atk = m.accelMassFiltered.lincurve(0, 5 * sens,0.02,0.001,-1);
	...
	var hh = m.accelMassFiltered.lincurve(0, 5 * sens,0,1.0,1);
	...
	var ca = m.accelMassFiltered.lincurve(0, 5 * sens,0.0,1.0,-12);
